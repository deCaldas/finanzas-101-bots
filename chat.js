...
function mostrarRespuestaChatbot(respuesta) {
  ...
  mensajeChatbot.textContent = `Bot: ${respuesta}`;
...
function borrarConversacion() {
  // Eliminar todo el contenido dentro del chatbox
  chatbox.innerHTML = '';
  ...
  // Escuchar el evento de presionar la tecla "Enter" en el campo de entrada
  // Para hacer esto, necesitamos agregar un event listener al campo de entrada, no al botón de borrar conversación
  Enterpress.addEventListener('keypress', (event) => {
    if (event.key === 'Enter') {
      borrarConversacion();
    }
  });
